...
## Score the LT-PLC upregulated human signature (bulk, GSVA/ssGSEA-style on
## TPM) per patient in Mulet-Lazaro et al. 2025's 30 ALAL (acute leukemia of
## ambiguous lineage) cohort, then group patients by the paper's own E-CAL
## transcriptional-classifier lineage assignment and WHO 2022 classification.
##
...
suppressPackageStartupMessages({
  library(dplyr); library(ggplot2); library(readxl); library(GSVA)
})
...
meta <- s3 %>% left_join(s13, by = "ID") %>% filter(!is.na(ID))
...
## ---- 3. score each patient with ssGSEA on log2(TPM+1) ----
...
logexpr <- log2(expr + 1)
gsvapar <- ssgseaParam(logexpr, sig, normalize = TRUE)
scores <- t(gsva(gsvapar))
...
p <- ggplot(plot_df, aes(x = ID, y = LT_PLC_up_all, fill = ecal_simple)) +
  ...
  labs(title = "Mulet-Lazaro et al. 2025: LT-PLC signature score across 30 ALAL patients",
       subtitle = "ssGSEA on bulk TPM; bars = up_all signature (2380 genes), open circles = up_top500 signature (378 genes);\npatients ordered by up_all score, colored by the paper's own E-CAL transcriptional lineage classifier",
       x = "Patient", y = "ssGSEA score (LT-PLC up)") +
